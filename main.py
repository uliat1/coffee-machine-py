class CoffeeMachine:
    ...
    def remaining(self):
        print(f"The coffee machine has: {self.water} of water, {self.milk} of milk, {self.money} of money, {self.coffee_beans} of coffee beans, {self.cups} of cups")

    def buy(self):
        choice = input("What do you want to buy?\n1 - espresso\n2 - latte\n3 - cappuccino\n")
        if choice == "1":
            self.make_coffee(250, 0, 16, 10, 1)
        elif choice == "2":
            self.make_coffee(350, 75, 20, 25, 1)
        elif choice == "3":
            self.make_coffee(200, 100, 12, 20, 1)
        else:
            print("Invalid choice!")

    def make_coffee(self, water_needed, milk_needed, coffee_beans_needed, cost, cups_needed):
        if self.water >= water_needed and self.milk >= milk_needed and self.coffee_beans >= coffee_beans_needed and self.cups >= cups_needed:
